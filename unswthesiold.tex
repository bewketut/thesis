...
(SMEs) and consumer services. This is because we can significantly reduce traﬃc
load if we use multicast eﬃciently as we will see in section 1.1 on the next page. Nevertheless, it has remained elusive to many Network Managers, Software Developers,
...
the one on the bottom, which is the essence of multicasting, the data is replicated
...
of hosts wanting that traﬃc. The disadvantage of broadcasting1 in relation can
...
multicast traffic. In this thesis, when we say firewall2 what we are referring
is the
...
• To protect client (insider) group members from unwanted traffic (individ-
ual tailored) from outside or within the network and
• Possibly, preventing an unauthorized user within or outside the network
...
multicas-
ting amplifies it because multiple resources can be members- (e.g an outsider
dumps
traffic on the stock exchange network). All innocent group members become
victim
as opposed to unicast where only one user4 is affected. To defend that, the
usual
...
re-
quirements in order to relieve the Network Manager from doing that:
...
al[8]. To
elaborate on step 2 a bit, it can, for example, be watching over all TCP
connections
on Port 80. That is when a user is simply browsing the web. We say more on
this
on later parts.
\section{Contribution}
Contribution of this thesis involves:
• An algorithm that avoids as many memory allocations as possible.
• A data structure that help models all entities involved and algorithms
along with it
• The overall design
...
• What kind of security model multicast entails and thus the paradigm we
follow?
...
or
so (e.g MSDP/MBGP/PIM in IPv4). This is only to stress multicasting is only a
model- not actual technology. So is unicasting but it perfectly maps to IP
without
...
• The router asks every period (of about 125 seconds) if anyone wants any
...
in IPv4.
• Hosts that want to join a group or keep their membership alive reply
(report) with the group address they are still participating or want to
Host IGMP Report
start participating.
• If the router receives no interest from at least a single host to be in a
...
%figure
The fact is we don’t even need to dissect the IGMP header to extract the group
...
• The type describes what kind of IGMP message it is. From it we know
...
• The max resp time is heuristic based constant, which tells the host about
...
MLDv2 is the coun-
• Only those unicast source addresses that one want to hear from. (Include
mode)
• All except those source addresses that one want hear from. (Exclude
mode)
The sudden complexity and bloatness of this design is worth as it is proven by
...
dissemination
type communication.3 But, multicasting has two other extra paradigms-many to
one and many to many- and as such it solves 1/3rd of the problem.
...
• Router maps IGMPv2 Joins (in SSM address range) to well- known sources
via DNS
• It usually allows only for one, or more, sources per group
• Router maps group to source (sources) – Uses either DNS or static internal
database
...
the
user has left with the intention to come back soon. For example, the user
could
...
that haunts our design to cause false positives2 is unicast snooping short-
fall. In Chapter 6, we outline ways to mitigate this problem of ’network
auto-immune disease’.
...
units
2\footnote{from firewall point of view, the positives are the ones that are tested to be
malicious}
...
plement the function themselves- it provides the prototype of the function though3.
...
• PREROUTING: All the packets, with no exceptions, hit this hook, which
...
hook4. This is the last hook in the incoming path for the local machine
traffic.
• FORWARD: Packets not going to the local machine (e.g., packets going
through the firewall) reach this hook. In a real system this will be how
3\footnote{This is C’s powerful way providing what one calls ’interface in object-oriented paradigm’}
4\footnote{As opposed to forwarding traffic.}
...
5\footnote{Jan have also graciously provided extensive tutorial on ’Writing Netfilter Modules’
...
We can prove the claim by assuming ’trust chaining6’ and using transitivity
property.
The way we implemented the above claim is:
6\footnote{This is how digital certificates work too.}
